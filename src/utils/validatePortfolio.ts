...
export function validProjects(value: unknown): value is Project[] {
  return Array.isArray(value) && new Set(value.map((x) => x?.id)).size === value.length && value.every((x) => record(x) && text(x.id) && text(x.title) && text(x.tag) && (x.categories === undefined || (Array.isArray(x.categories) && x.categories.every(text))) && text(x.stack) && text(x.description) && safeLink(x.link) && safeImage(x.image) && optionalText(x.role) && (x.status === undefined || x.status === 'In progress' || x.status === 'Completed'));
}
export function validOrganizations(value: unknown): value is OrgExp[] {
  return Array.isArray(value) && new Set(value.map((x) => x?.id)).size === value.length && value.every((x) => record(x) && typeof x.id === 'number' && Number.isFinite(x.id) && text(x.org) && text(x.period) && [x.role, x.location, x.summary, x.imageCaption].every(optionalText) && safeImage(x.image) && (x.stack === undefined || (Array.isArray(x.stack) && x.stack.every(text))) && (x.projects === undefined || (Array.isArray(x.projects) && x.projects.every((p) => record(p) && text(p.id) && text(p.title) && text(p.subtitle) && text(p.description) && safeImage(p.image) && Array.isArray(p.stack) && p.stack.every(text)))) && (x.imageLayout === undefined || x.imageLayout === 'original' || x.imageLayout === 'landscape') && (x.roles === undefined || (Array.isArray(x.roles) && x.roles.every((r) => record(r) && text(r.title) && optionalText(r.context) && Array.isArray(r.bullets) && r.bullets.every(text)))));
}
