...
test('chat validates requests, handles provider failures, and limits repeated calls', async () => {
  const originalKey=process.env.OPEN_ROUTER;
  const originalModel=process.env.OPEN_ROUTER_MODEL;
  ...
  process.env.OPEN_ROUTER='test-placeholder';
  ...
  const mocked=mock.method(globalThis,'fetch',async (url,init)=>{
    ...
    assert.equal(payload.model,process.env.OPEN_ROUTER_MODEL || 'openrouter/free');
  ...
  try {
    ...
    process.env.OPEN_ROUTER_MODEL='test/model:free';assert.equal((await call('custom',{ip:'custom'})).code,200);delete process.env.OPEN_ROUTER_MODEL;
    ...
    mocked.mock.restore();
    if(originalKey===undefined)delete process.env.OPEN_ROUTER;else process.env.OPEN_ROUTER=originalKey;
    if(originalModel===undefined)delete process.env.OPEN_ROUTER_MODEL;else process.env.OPEN_ROUTER_MODEL=originalModel;
  }
});
